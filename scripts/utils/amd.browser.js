...
(function (root) {
  ...
  function _loadPluginModule(
     ...
     */
    pluginPath =
      (context ? context + "/" : "") + "plugins/" + module.replace(/\//g, "_");
   ...
   */
  function _resolveCircularReferences(
    id,
    dependencies,
    circulars,
    i,
  ...
  function define(
    ...
    if (
      ...
      factory
        ...
        .replace(
          /(?:require)\(\s*["']([^'"\s]+)["']\s*\)/g, // Now check for any require("module") calls
  ...
  require.config = function (obj) {
    ...
    _baseUrl +=
      _baseUrl && _baseUrl.charAt(_baseUrl.length - 1) !== "/" ? "/" : "";
